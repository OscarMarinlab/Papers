...
library (ggplot2) 
library (plyr)
DATA1<- read.csv("neto1_e16.csv")

DATA1$Genotype <- factor(DATA1$Genotype, levels = c("Neto1+/+", "Neto1+/-", "Neto1-/-"))
##
...
mm1mz <- subset(mm1, Region == "MZ" & (Genotype =="Neto1+/-" | Genotype =="Neto1-/-"))
mm1mz <- droplevels(mm1mz)
