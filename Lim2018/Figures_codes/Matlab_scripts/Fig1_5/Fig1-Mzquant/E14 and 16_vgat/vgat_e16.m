...
function data = vgat_e16(filename, A)
...
se2 = strel('disk', 1,8); %structral element 
se2B = strel ('disk',1,8);
...
fgm2 = bwareaopen(fgm,1); %gating for size
figure, imshow(fgm2);
%%
[~, bw_gfp] = thresh_tool(imadjust(I_gfp)); %image to black and white - binary operation
%bw_gfp =  bwareaopen (bw_gfp, 40);
                 
bw_total = bw_gfp & fgm2;
%bw_total = bwareaopen (bw_total, 5); 
%figure, imshow(bw_total); 
bw_gfp = bwareaopen(bw_gfp,5); % morphological opening to gate for the miniminal size of objects
figure, imshow(bw_gfp); 
